...
One strength of the *ETB* is its rich, but easy accessible set of functionalities.
...
* Now enter `cd 28-011927fdb603` (or whatever address the sensor has on your system)
* Enter `cat w1_slave` which will show the raw temperature reading output by the sensor, where the temperature reading is `t=28625`, which means a temperature of 28.625 degree Celsius.
...
* To check if an TWI/I2C device is available, enter:  
    `sudo i2cdetect -y 1`  
  ...
  In MySQL 5.7 (released Oct 2015) and MySQL 8, the root MySQL user is set to authenticate using the auth_socket or caching_sha2_password plugin rather than with mysql_native_password. This will prevent programs like phpMyAdmin from logging in with the root account.
    ...
    * Create a New Superuser for phpMyAdmin:
        ...
        * Create a new superuser (with local access only):
          `CREATE USER 'admin'@'localhost' IDENTIFIED BY '$MyWSNdemo$';`
...
* Create an own user for the WSN testbed:
  `CREATE USER 'mywsn' IDENTIFIED BY '$MyWSNdemo$';`
...
Further configuration and setup of the databases is done via `PHPmyAdmin`.
...
```
* To configure the network interfaces, create a local config like:
...
```
    host climatechamber {
        hardware ethernet 00:d0:c9:d9:eb:3e;
