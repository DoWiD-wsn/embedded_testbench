...
class ina219(object):
    ...
    def calibrate_16V_400mA(self):
        ...
        self.set_bus_RNG(self._BRNG_16)
        self.set_PGA(self._PG_1)
        self.set_bus_ADC(self._ADC_BITS_12, self._ADC_SAMPLE_1)
        self.set_shunt_ADC(self._ADC_BITS_12, self._ADC_SAMPLE_1)
        self.set_mode(self._MODE_SB_CONT)
    ...
    def reset(self):
        ...
        self.write_register(self._REG_CONFIG, self._RST)
    ...
    def get_power_W(self):
        return float(self.read_ina_register(self._REG_POWER)) * self._power_lsb
    ...
    def get_power_mW(self):
        return float(self.read_ina_register(self._REG_POWER)) * (self._power_lsb*1000)
    ...
    def set_cal_register(self, value):
        self.write_register(self._REG_CALIBRATION, value)
    ...
    def set_bus_RNG(self, brng):
        ...
        if (brng<self._BRNG_16) or (brng>self._BRNG_32):
            return
        # Read config register value
        reg = self.read_ina_register(self._REG_CONFIG)
        # Prepare new register value
        conf = (reg & 0xDFFF) | (brng<<13)
        # Write new register value
        self.write_register(self._REG_CONFIG, conf)
    ...
    def set_PGA(self, pg):
        # Check the given value
        if (pg<self._PG_1) or (pg>self._PG_8):
            return
        # Read config register value
        reg = self.read_ina_register(self._REG_CONFIG)
        # Prepare new register value
    ...
    def set_bus_ADC(self, bits, sample):
        # Check the given value
        if (bits<self._ADC_BITS_9) or (bits>self._ADC_BITS_12):
            return
        if (sample<self._ADC_SAMPLE_1) or (sample>self._ADC_SAMPLE_128):
            return
        # Get new ADC configuration
        value = 0
        if(bits < self._ADC_BITS_12):
            value = bits
        else:
            value = 0x80 | sample
        # Read config register value
    ...
    def set_mode(self, mode):
        # Check the given value
        if (mode<self._MODE_PDOWN) or (mode>self._MODE_SB_CONT):
            return
        # Read config register value
        reg = self.read_ina_register(self._REG_CONFIG)
        # Prepare new register value
